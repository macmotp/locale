...
final class SM extends AbstractCountry implements CountryInterface
{
    ...
    public function __construct()
    {
        ...
        $this->setName(CountryName::SAN_MARINO);
        $this->setCapital(CountryCapital::SAN_MARINO);
        $this->setCode(CountryCode::SM);
        $this->setAlpha3Code('SMR');
        $this->setDialCode('+378');
        $this->setTld('.sm');
        $this->setDateFormat('d-m-Y');
        $this->setFlag(CountryFlag::SM);
        $this->setDefaultCurrency(CurrencyCode::EUR);
        $this->setDefaultTimezone(TimezoneCode::EUROPE_SAN_MARINO);
        $this->setDefaultLanguage(LanguageCode::ITALIAN);
        $this->setCurrencies(new Collection([CurrencyCode::EUR,]));
        $this->setTimezones(new Collection([TimezoneCode::EUROPE_SAN_MARINO,]));
