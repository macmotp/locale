...
final class JE extends AbstractCountry implements CountryInterface
{
    ...
    public function __construct()
    {
        ...
        $this->setName(CountryName::JERSEY);
        $this->setCapital(CountryCapital::SAINT_HELIER);
        $this->setCode(CountryCode::JE);
        $this->setAlpha3Code('JEY');
        $this->setDialCode('+44');
        $this->setTld('.je');
        $this->setDateFormat('d-m-Y');
        $this->setFlag(CountryFlag::JE);
        $this->setDefaultCurrency(CurrencyCode::GBP);
        $this->setDefaultTimezone(TimezoneCode::EUROPE_JERSEY);
        $this->setDefaultLanguage(LanguageCode::ENGLISH);
        $this->setCurrencies(new Collection([CurrencyCode::GBP,]));
        $this->setTimezones(new Collection([TimezoneCode::EUROPE_JERSEY,]));
