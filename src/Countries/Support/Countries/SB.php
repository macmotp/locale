...
final class SB extends AbstractCountry implements CountryInterface
{
    ...
    public function __construct()
    {
        ...
        $this->setName(CountryName::SOLOMON_ISLANDS);
        $this->setCapital(CountryCapital::HONIARA);
        $this->setCode(CountryCode::SB);
        $this->setAlpha3Code('SLB');
        $this->setDialCode('+677');
        $this->setTld('.sb');
        $this->setDateFormat('d-m-Y');
        $this->setFlag(CountryFlag::SB);
        $this->setDefaultCurrency(CurrencyCode::AUD);
        $this->setDefaultTimezone(TimezoneCode::PACIFIC_GUADALCANAL);
        $this->setDefaultLanguage(LanguageCode::ENGLISH);
        $this->setCurrencies(new Collection([CurrencyCode::AUD,]));
        $this->setTimezones(new Collection([TimezoneCode::PACIFIC_GUADALCANAL,]));
