...
final class GP extends AbstractCountry implements CountryInterface
{
    ...
    public function __construct()
    {
        ...
        $this->setName(CountryName::GUADELOUPE);
        $this->setCapital(CountryCapital::BASSE_TERRE);
        $this->setCode(CountryCode::GP);
        $this->setAlpha3Code('GLP');
        $this->setDialCode('+590');
        $this->setTld('.gp');
        $this->setDateFormat('d-m-Y');
        $this->setFlag(CountryFlag::GP);
        $this->setDefaultCurrency(CurrencyCode::EUR);
        $this->setDefaultTimezone(TimezoneCode::AMERICA_GUADELOUPE);
        $this->setDefaultLanguage(LanguageCode::FRENCH);
        $this->setCurrencies(new Collection([CurrencyCode::EUR,]));
        $this->setTimezones(new Collection([TimezoneCode::AMERICA_GUADELOUPE,]));
