...
final class GA extends AbstractCountry implements CountryInterface
{
    ...
    public function __construct()
    {
        ...
        $this->setName(CountryName::GABON);
        $this->setCapital(CountryCapital::LIBREVILLE);
        $this->setCode(CountryCode::GA);
        $this->setAlpha3Code('GAB');
        $this->setDialCode('+241');
        $this->setTld('.ga');
        $this->setDateFormat('d-m-Y');
        $this->setFlag(CountryFlag::GA);
        $this->setDefaultCurrency(CurrencyCode::XAF);
        $this->setDefaultTimezone(TimezoneCode::AFRICA_LIBREVILLE);
        $this->setDefaultLanguage(LanguageCode::FRENCH);
        $this->setCurrencies(new Collection([CurrencyCode::XAF,]));
        $this->setTimezones(new Collection([TimezoneCode::AFRICA_LIBREVILLE,]));
