...
final class TC extends AbstractCountry implements CountryInterface
{
    ...
    public function __construct()
    {
        ...
        $this->setName(CountryName::TURKS_AND_CAICOS_ISLANDS);
        $this->setCapital(CountryCapital::COCKBURN_TOWN);
        $this->setCode(CountryCode::TC);
        $this->setAlpha3Code('TCA');
        $this->setDialCode('+1649');
        $this->setTld('.tc');
        $this->setDateFormat('d-m-Y');
        $this->setFlag(CountryFlag::TC);
        $this->setDefaultCurrency(CurrencyCode::USD);
        $this->setDefaultTimezone(TimezoneCode::AMERICA_GRAND_TURK);
        $this->setDefaultLanguage(LanguageCode::ENGLISH);
        $this->setCurrencies(new Collection([CurrencyCode::USD,]));
        $this->setTimezones(new Collection([TimezoneCode::AMERICA_GRAND_TURK,]));
