...
final class NL extends AbstractCountry implements CountryInterface
{
    ...
    public function __construct()
    {
        ...
        $this->setName(CountryName::NETHERLANDS);
        $this->setCapital(CountryCapital::AMSTERDAM);
        $this->setCode(CountryCode::NL);
        $this->setAlpha3Code('NLD');
        $this->setDialCode('+31');
        $this->setTld('.nl');
        $this->setDateFormat('d-m-Y');
        $this->setFlag(CountryFlag::NL);
        $this->setDefaultCurrency(CurrencyCode::EUR);
        $this->setDefaultTimezone(TimezoneCode::EUROPE_AMSTERDAM);
        $this->setDefaultLanguage(LanguageCode::DUTCH);
        $this->setCurrencies(new Collection([CurrencyCode::EUR,]));
        $this->setTimezones(new Collection([TimezoneCode::EUROPE_AMSTERDAM,]));
