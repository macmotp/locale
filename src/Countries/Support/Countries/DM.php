...
final class DM extends AbstractCountry implements CountryInterface
{
    ...
    public function __construct()
    {
        ...
        $this->setName(CountryName::DOMINICA);
        $this->setCapital(CountryCapital::ROSEAU);
        $this->setCode(CountryCode::DM);
        $this->setAlpha3Code('DMA');
        $this->setDialCode('+1767');
        $this->setTld('.dm');
        $this->setDateFormat('d-m-Y');
        $this->setFlag(CountryFlag::DM);
        $this->setDefaultCurrency(CurrencyCode::XCD);
        $this->setDefaultTimezone(TimezoneCode::AMERICA_DOMINICA);
        $this->setDefaultLanguage(LanguageCode::ENGLISH);
        $this->setCurrencies(new Collection([CurrencyCode::XCD,]));
        $this->setTimezones(new Collection([TimezoneCode::AMERICA_DOMINICA,]));
